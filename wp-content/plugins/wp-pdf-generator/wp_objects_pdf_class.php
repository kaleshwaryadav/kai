...
if(!class_exists('WPEXPERTS_PDF')){
    ...
    class WPEXPERTS_PDF{
        ...
        public function wpexperts_pdf_init(){
            ...
            add_action( 'wp_head', array( $this, 'wpexperts_pdf_scripts'), 1001 );
            add_action( 'admin_menu', array( $this, 'wpexperts_pdf_setting_page' ) );
            add_action( 'wp_head', array ( $this, 'wpexperts_pdf_generate_PDF' ) );
        ...
        public function wpexperts_pdf_menu_page(){
            ...
            echo '<input type="text" name="wpexperts_pdf_text" value="'.$wpexperts_pdf_btn_text.'" placeholder="Enter Your Button Text here" /><hr />';
        ...
        private function wpexperts_pdf_heading_radio_buttons( $wpexperts_pdf_name , $wpexperts_pdf_value ){
            ...
            echo '<input type="radio" name="wpexperts-pdf-select" value="wpexperts-pdf-head-'. $wpexperts_pdf_value .'" id="' . $wpexperts_pdf_value . '"  '. $this->wpexperts_pdf_radio_checked( $wpexperts_pdf_value ) .'/></div>';
            echo '<div class="btn-pfd-text"><label for="' . $wpexperts_pdf_value . '">' . $wpexperts_pdf_name . '</label></div></div>';
        }
        ...
        private function wpexperts_pdf_page_radio_buttons( $wpexperts_pdf_name = null , $wpexperts_pdf_value ){
            echo '<div class="wpexperts_pdf_head"><div class="btn-box-head">';
            echo '<input type="radio" name="wpexperts-pdf-page" value="'. $wpexperts_pdf_value .'" id="' . $wpexperts_pdf_value . '"  '. $this->wpexperts_pdf_radio_checked( $wpexperts_pdf_value ) .'/></div>';
            echo '<div class="btn-pfd-text"><label for="' . $wpexperts_pdf_value . '">' . $wpexperts_pdf_name . '</label></div></div>';
        }
        ...
        private function wpexperts_pdf_page_size_radio_buttons( $wpexperts_pdf_name = null , $wpexperts_pdf_value ){
            echo '<div class="wpexperts_pdf_head"><div class="btn-box-head">';
            echo '<input type="radio" name="wpexperts-pdf-page-size" value="'. $wpexperts_pdf_value .'" id="' . $wpexperts_pdf_value . '"  '. $this->wpexperts_pdf_radio_checked( $wpexperts_pdf_value ) .'/></div>';
            echo '<div class="btn-pfd-text"><label for="' . $wpexperts_pdf_value . '">' . $wpexperts_pdf_name . '</label></div></div>';
        }
        ...
        private function wpexperts_pdf_radio_buttons(){
            ...
            for ($wpexperts_pdf = 1; $wpexperts_pdf <= 5; $wpexperts_pdf++) {
                ...
                echo '<label for="wpexperts_pdf_set-'. $wpexperts_pdf .'"><img src="'. WPEXPERTS_PDF_ASSETS . 'images/button/wpexperts-pdf-' . $wpexperts_pdf .'.jpg" width="25" /></label>';
                echo '<div class="btn-pfd-input"><input type="radio" name="wpexperts-pdf" value="wpexperts-pdf-'. $wpexperts_pdf .'" id="wpexperts_pdf_set-'. $wpexperts_pdf .'" '. $this->wpexperts_pdf_radio_checked( $wpexperts_pdf ) .' /></div></div>';
            }
            echo '</div><hr />';
        }
        ...
        private function wpexperts_pdf_save_data(){
            if(isset($_POST['wpexperts_pdf_save'])){
                $wpexperts_pdf_style        = $_POST['wpexperts-pdf-select']; 
                $wpexperts_pdf_btn_set      = $_POST['wpexperts-pdf']; 
                $wpexperts_pdf_text         = $_POST['wpexperts_pdf_text']; 
                $wpexperts_pdf_page         = $_POST['wpexperts-pdf-page'];
                $wpexperts_pdf_page_size    = $_POST['wpexperts-pdf-page-size'];
            ...
            <script>
               jQuery(document).ready(function($) {
                    ...
                    $( "#wpexperts_pdf_generate_file" ).click(function() {
                        ...
                        var opt = {
                            margin:       1,
                            filename:     'wpexperts-<?php echo $post->post_name; ?>' + '.pdf',
                            image:        { type: 'jpeg', quality: 0.98 },
                            html2canvas:  { scale: 2 },
                            jsPDF:        { unit: 'pt', format: '<?php echo $wpexperts_pdf_page_size; ?>', orientation: '<?php echo $wpexperts_pdf_page; ?>' }
                        };
        ...
        public function wpexperts_pdf_shortcode(){ 
            ...
            echo '<a class="button" id="wpexperts_pdf_generate_file">';
                if( $wpexperts_pdf_btn_style == 'wpexperts-pdf-head-image-btn' ){
                    echo '<img src="'. WPEXPERTS_PDF_ASSETS . 'images/button/' . $wpexperts_pdf_btn_set . '.jpg" width="25" />';
                }
                
                if( $wpexperts_pdf_btn_style == 'wpexperts-pdf-head-text-btn' ){
                    echo $wpexperts_pdf_btn_text;
                }
            echo '</a>';
