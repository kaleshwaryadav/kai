...
class standard_php_library__PuvoxSoftware 
{
	...
	public function __construct($args=[])
	{
		...
		if (!$this->isWP || array_key_exists('custom_urls', $args))
		{
			$this->wpURL 			= "";
			$this->wpFOLDER 		= "";
			$this->homeURL 			= $this->wpURL;
			$this->homeFOLDER 		= $this->wpFOLDER;
			$this->moduleURL		= $this->wpURL;
		}
		...
		$this->Default_Post_Thumb_Imagee= 'data:image/svg+xml;utf8,&lt;svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1 1"&gt;&lt;/svg&gt;';
		$this->site_favicon 	= 'data:image/svg+xml;utf8,&lt;svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1 1"&gt;&lt;/svg&gt;'; 

		$this->setupBaseUrls();  //because we can later resetup them
	...
	public function tmp_name($name){
		return $this->baseDIR.'/_temp/' . $name.'.html';  //$_SERVER['DOCUMENT_ROOT']
	}
	...
	public function get_option_json($name, $default_value=null){
		$json = file_exists($path = $this->baseDIR.'/site_options.ini') ? $this->file_get_contents($path) : '{}';
	...
	public function fileUrl($file){
		return $this->moduleURL."/$file?vers_=".$this->filedate($this->baseDIR. "/$file");
	}
	...
	public function wpdb_new($path_to_root= '/../../../../../../'){
		...
		if(!defined('DB_HOST')){ 
			$content=file_get_contents($path_to_wp.'/wp-config.php');
	...
	public function error_notify_admin__MYDDD($error_msg=false,$postidd=false){ 	if (test_environment) return;
		...
		//write into file
			//$file=$this->baseDIR.'/zzz___ajax_error_notifications_'.$this->my_site_variables__secret('rand_name', RandomString(11)).'.txt';
...
var PuvoxLibrary = 
{
	...
	jquery_popup(element, isModal, params){
		...
		var height = params.hasOwnProperty('height') ? params.height : 200;
	...
	jquery_popup_once(cookiename, key, text, duration, onComplete){
		var text = text;
		var cookieVal = this.getCookieOption(cookiename, "popup_shown_"+key, "true");
		var onComplete = onComplete || function(){};
		if(cookieVal=="true")
		{
			text = '<div>'+text+'</div>';
			var pp = this.jquery_popup(text, true, {});
			this.setCookieOption(cookiename, "popup_shown_"+key, "false"); 
	...
	jquery_popup_one_time_checkbox(cookiename, key, text, defaultCheckboxTxt){
		var defaultCheckboxTxt = defaultCheckboxTxt || '<div class="dialog_dont_show_again" style="position:absolute; bottom:0; right:0; font-size:0.7em;"><input type="checkbox" onclick="PuvoxLibrary.dialog_dont_show_again(event, \''+key+'\', \''+cookiename+'\')" /><span class="dont_show_again_inner">Don\'t show this window again</span></div>';
		var text = text	+ defaultCheckboxTxt;
		var cookieVal = this.getCookieOption(cookiename, "popup_checkbox_"+key, "true");
		if(cookieVal=="true")
		{
			text = '<div>'+text+'</div>';
			var pp = this.jquery_popup(text, true, {});
	...
	{
		this.setCookieOption(cookiename, "popup_checkbox_"+key, (event.target.checked ? "false" : "true") ); 
	},
	...
	toggleWindowsMessages_WindowConfirm	: window.confirm,
	toggleWindowsMessages_WindowAlert	: window.alert,
...
class standard_wp_library__PuvoxSoftware extends standard_php_library__PuvoxSoftware  
{
	...
	public function loadScripsStyles()
	{
		...
		$this->autogenerated_scripts_REGISTER();
	}
	...
	public function autogenerated_scripts_REGISTER() 
	{
		add_action( 'wp_enqueue_scripts',		[$this,'autogenerated_scripts_enqueue'], 999 );
		$sURL= $this->baseScriptsUrl;
		$sDIR= $this->baseScriptsDir . '/';
		$this->autogenerated_scripts_names		= ['js'=> 'script-autogenerated.js', 'css'=>'style-autogenerated.css']; //plugins_url('file.js', __FILE__)
	...
	public function autogenerated_scripts_write() 
	{
		...
		if (!empty($this->autogenerated_scripts_contents['js']))
		{
			if( !file_exists($this->autogenerated_scripts_paths['js'] ) || $this->autogenerated_scripts_contents['js']!= file_get_contents($this->autogenerated_scripts_paths['js']))	file_put_contents($this->autogenerated_scripts_paths['js'], $this->autogenerated_scripts_contents['js']);
		}
		if (!empty($this->autogenerated_scripts_contents['js']))
		{
			if( !file_exists($this->autogenerated_scripts_paths['css'] ) || $this->autogenerated_scripts_contents['css']!= file_get_contents($this->autogenerated_scripts_paths['css']))	file_put_contents($this->autogenerated_scripts_paths['css'], $this->autogenerated_scripts_contents['css']);
		}
	}
...
class default_plugin__PuvoxSoftware 
{
	...
	{
		$this->helpers = new standard_wp_library__PuvoxSoftware(['class'=>get_called_class()] + $arg1);
	...
	{			
		if ($this->helpers->isWP)
		{
			...
			$this->plugin_page_url	= ( !is_network_admin() && (!is_multisite() || !$this->network_managed || !$this->IsNetworkwidePlugin() || !is_plugin_active_for_network($this->static_settings['plugin_basename']) ) ? admin_url() : network_admin_url() ) .( !empty($this->static_settings['custom_opts_page']) ?  $this->static_settings['custom_opts_page'] : $this->settingsPHP_page.'?page='.$this->optionsPageSlug); 
	...
	public function my_plugin_vars($step=0)
	{
		...
		if(method_exists($this,'declare_settings') ) 
		{
			...
			$this->initial_static_options = $this->initial_static_options + $temp2;
		}
		else{
			$this->initial_user_options=[]; 
			$this->initial_static_options = $temp2;
		}
		$this->static_settings	= array_merge($this->initial_static_options, $temp1);
	}
	...
	public function flush_rules_if_needed($temp_key=false){
		...
		$key="b".get_current_blog_id()."_". md5(    (empty($temp_key) ?  "sample" : ( stripos($temp_key, basename($this->helpers->baseDIR)) !== false ? md5(filemtime($temp_key)) : $temp_key ))    );
	...
	*/
	...
	{ 
		...
		{
			$cont .= $this->file_get_contents($this->helpers->baseDIR.'/'.$each);
		}
		...
		<div class="in_additional">
			...
			<ul class="donations_block">
				<li class="donation_li">
					...
					<?php _e('If you found this plugin useful, any minimal');?> <?php echo $this->paypalDonationButton();?> <?php _e(' is welcomed');?> 
					<script>
...
trait pro_plugin__PuvoxSoftware
{
	...
	*/
	...
	public function paypalDonationButton(){ return '<a class="button" style="display:inline-block; line-height:1em; min-height:25px; color:#179bd7; " href="void(0);" onclick="tt_donate_trigger(event);"/> <img style="height:20px; vertical-align:middle;" src="'.  $this->helpers->imageSvg("paypal") .'" /> '. __("donation") .'</a>'; }
	...
	public function donations_trigger_popup()
	{
		...
		if ( $this->static_settings['show_donation_popup'] && ! $this->helpers->valueIs($this->opts, 'donate_popup_a2', '1d') )
		{
			...
			if (  $this->opts['first_install_date'] != $this->opts['last_update_time'] )
			{
				...
				$this->update_opts();
				$text = sprintf(__('Dear users, our plugin (<code>%s</code>) is free.  However, every plugin needs a noticeable amount of work to develop it. <br/>If you found this plugin useful, your minimal %s will support us (developers) to maintain this plugin and keep it functional. Thank you.'), $this->static_settings['Name'], $this->paypalDonationButton() );
				?>
