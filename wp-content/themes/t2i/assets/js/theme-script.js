jQuery(document).ready(function($) {
    ...
    if(getLangCode == 'en')
    {
        ...
        var user_message_note ="This field is required.";
    }
    else
    {
...
/* Send message to assert owner */
 ...
 $('#shareemail').validate({ // initialize the plugin
   ...
   $('#assetOwnerContact').click(function(){
    //console.log("fdsafdasfas");
        $('#owner_message_note').html("");
        var formdata = $('#userContactAssetOwner');
            $('#owner_message_note').html();
            $('#loading-image').show();  
            $.ajax({            
                type: 'POST',
                ...
                success: function (data) {
                if (data.status == "Success"){
                    $('#owner_message_note').html(data.message);
                    //$('#owner_note').val("");
                    $("#userContactAssetOwner").trigger("reset");                   
                    //$('.post_status').html(data.message);
                    //$('#loading-image').hide(); 
                }
                else {
                    $('#owner_message_note').html(data.message);
                    $('#owner_message_note').fadeOut(3000); 
                }
                }
            });
            return false; // blocks redirect after submission via ajax
...
$('#userPrivateNoteButton').click(function(){
    var form_id = $('#userPrivateNote');
    $('#user_note').val("");
    $('#user_message_note').html();
    $('#loading-image').show();  
    $.ajax({            
                type: 'POST',
                dataType: 'json',
                ...
                success: function (data) {
                if (data.status == "Success"){
                    $('#user_message_note').html(data.message);
                    $('#user_note').val("");
                    $("#userPrivateNote").trigger("reset");                   
                    //$('.post_status').html(data.message);
                    //$('#loading-image').hide(); 
                }
                else {
                    $('#user_message_note').html(data.message);
                    $('#user_message_note').fadeOut(3000); 
                    //$('#loading-image').hide(); 
                    }
                }
            });
