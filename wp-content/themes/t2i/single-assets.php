...
<script type="text/javascript">
  var asset_id = <?php echo get_the_ID(); ?>;
</script>
<?php 
...
<div class="template-wrapper extended">
    <div>
        <div class="container">
            ...
            <div class="product_details ">
                    ...
                    <div class="col-sm-6">
                         ...
                         <h4><?php the_field('name', get_the_ID()); ?></h4>
                           ...
                           <?php //wp_editor( wpautop(get_option('content',get_field('short_description',get_the_ID()))), 'content', $settings ); ?>
                            <div class="features">
                            <?php  $asset_url = get_field('asset_link', get_the_ID(),true);
                            $asset_link = url_parser($asset_url);
                             if(!empty($asset_link)){
                ...
                $temp_description = get_field('temp_description',get_the_ID());
                $desc_second = get_field('description_business',get_the_ID()); 
                ?>
               
                <?php if(!empty($temp_description)){ ?>
                <label>Asset Description</label>
                <p><?php echo $temp_description; ?></p>
                <?php } ?>
                <br/><br/>
                
                <?php if(!empty($desc_second)){ ?>
                  <label>Business Description</label>
                <p><?php echo $desc_second; ?></p>
                <?php } ?>
              </div>
             <?php 
             $owneruser  =  get_post($post->ID);
             $user_id = $owneruser->post_author;
             $user_data = get_user_by('id', $user_id);
             $UserPic = get_user_meta($user_id,'user_picImage',true);              
             $user_roles = $current_user->roles;
             $first_name = $user_data->data->user_nicename;
             ...
             if($lanCode=='en'){
                ...
                $more_detail = "Contact to Owner";
                $user_note = "Private Note";
                $readonly = "Sorry! Only registered user allow to send message";
                $detail = "Detail";
                $no_available = "There are no Notes available currently";
                $PersontoownerNotes ="User to Asset Owner Notes";
                $userNotes ="My Notes";
                $location_text = "Location";
                $ContactUsQuery = "Contact Us";
             }
             else {
              ...
              <div class="row no-margin">
                ...
                <div class="col-md-5 col-sm-5 col-xs-12 no-padding">
                    ...
                    <?php if(!empty($UserPic)){?>  
                         <img class="img-responsive center-block" src="<?php echo $UserPic; ?>" alt="" width="94" height="95">
                        <?php } else { ?>
                        ...
                        <div class="details_owner ">
                             ...
                             <div class="media-body">
                                ...
                                <a href="#" class="btn-info-note" data-toggle="modal" data-target="#usernote" ><?php echo $user_note; ?></a>
                                <?php }?>
                            </div>
                      ...
                      <?php
                        $UserToOwnerData = GetUserNotes($post->ID,'user_note');
                        if(!empty($UserToOwnerData)) {?>
                        <div class="private_notes fullwidth">
                        <h4><strong> <?php echo $userNotes; ?>:</strong></h4>
                        <div class="col-xs-12 usernotes">
                        <?php 
                          foreach($UserToOwnerData as $notes){ 
                          $user = get_user_by('id', $notes['user_id']); 
                          ?>
                          <blockquote class="wp-block-quote">
                            <span><?php echo $notes['message']; ?></span>
                            <small><?php echo date('d-m-Y', strtotime($notes['date'])); ?></small>
                          </blockquote>
                          </br>
                    ...
                    <?php 
                        ...
                        <div class="message-form-owner fullwidth">
                          <h4><strong> <?php echo $ContactUsQuery; ?>:</strong></h4>
                    ...
                    <div class="controls">
                            ...
                            <div class="col-lg-6">
                                <div class="form-group">                                    
                                    <input id="form_phone" type="text" name="m_number" class="form-control w-100" placeholder="<?php echo $mobile_placeholder; ?>  *" value="<?php echo (!empty($mobilenum)) ? $mobilenum : ''; ?>">
                    ...
                    <div class="contact-form fullwidth">
                       <h4><strong> <?php echo $PersontoownerNotes; ?>:</strong></h4>
                        <div class="col-xs-12 usernotes">
                        <?php $UserToOwnerData = GetContactToAssetOwnerLogs($post->ID,'user_note');
                        //print_r($UserToOwnerData);
                        if(!empty($UserToOwnerData)) { 
    ...
    <div id="moredetail" class="modal fade" role="dialog">
        <div class="modal-dialog">
            ...
            <div class="modal-content">
              ...
              $sql = "SELECT * FROM $table_name where user_id='$user_id' and post_id = '$post_id' ORDER BY id DESC";
              $UserAccess = $wpdb->get_row($sql);
                ...
                <div class="modal-body">
                    <div class="form_div feedback">
                        <div class="row">
                            ...
                            <div class="status" style="text-align:center;"></div>
                             <?php if(is_user_logged_in()){ ?>
                             <form id="userContactAssetOwner" class="userContactAssetOwner">
                              <input type="hidden" name="post_id" value="<?php echo get_the_ID(); ?>">
                              <input type="hidden" name="user_id" value="<?php echo $user_id; ?>">
                              <input type="hidden" name="action" value="sendMessageToAssetOwner">
                              <input type="hidden" name="type" value="asset_owner">
                              <div class="form-group1">
                              <span><b>Contact to Asset Owner:</b></span>  <span id="owner_message_note"></span> 
                              <textarea name="owner_message" class="form-control-new" cols="4" rows="4" id="owner_message" ></textarea>
                              </div>
                              </br>
                              <button id="assetOwnerContact" type="button" class="btn"><?php echo $save; ?></button>    
                              </form>
                                <?php } else {
                                   echo $readonly;
                                } ?>
    ...
    <div id="usernote" class="modal fade" role="dialog">
        <div class="modal-dialog">
            <!-- Modal content-->
            <div class="modal-content">
              <div class="modal-header">
              <button onclick="javascript:window.location.reload()" type="button" class="close" data-dismiss="modal">&times;</button>
                ...
                <div class="modal-bodydd">
                    <form class="userPrivateNote" id="userPrivateNote">
                    <input type="hidden" name="post_id" value="<?php echo get_the_ID(); ?>">
                    <input type="hidden" name="user_id" value="<?php echo $user_id; ?>">
                    <input type="hidden" name="action" value="accessLevel">
                    <input type="hidden" name="type" value="user_note">
                    <div class="form-group">
                        <span></span><b><?php echo $user_note; ?> :</b></span>  <span id="user_message_note"></span>
                        <textarea class="form-control-new" cols="4" rows="4" name="user_note"></textarea>
                    </div>
                    <button type="button" class="btn reminder" id="userPrivateNoteButton"><?php echo $save; ?></button>    
                    <form>
                </div>
            </div>
        </div>
