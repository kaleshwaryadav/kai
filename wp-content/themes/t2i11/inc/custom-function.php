 ...
 */
function GetUserNotes($postid){
    global $current_user,$wpdb;
    $table_name = $wpdb->prefix . "user_asset_accesslevel";
    $user_id = get_current_user_id() ;
    $UserNotes = $wpdb->get_results("SELECT * FROM $table_name where post_id = '$postid' and user_id NOT IN ($user_id) ORDER BY id DESC", ARRAY_A);
    return $UserNotes;
}
