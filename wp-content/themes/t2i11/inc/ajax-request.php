...
class Ajax_request_handle {
    ...
    */
    function ajaxlogin() {
          ...
          function rs_user_registration_callback(){
           ...
           if( is_wp_error( $new_user_id ) ) {
            ...
            if($new_user_id) {
              ...
              $success = 1;
              $this->send_mail($user_email,$user_login,$user_pass,$activation_link);
              die;
           }
          ...
          public function send_mail($email,$ursername,$password,$activation_link){
            $lanCode = ICL_LANGUAGE_CODE;
            if($lanCode=='en'){
...
*/
       ...
       /* $logo = 'http://d3.iworklab.com/t2iwp1/wp-content/uploads/2018/01/kai_logo.png';
        ...
        $reg_email_templ = icl_object_id(2247,'post',false,ICL_LANGUAGE_CODE);
       ...
       /* $message = '<table  cellspacing="0" cellpadding="0" border="0"  style="margin:0 auto; width:100%; max-width:650px;" class="wrapper">
        ...
        $mail = wp_mail($to, $subject, $message, $headers);*/
            ...
            function send_msg(){
              ...
              $current_user = wp_get_current_user();
                echo $lanCode = ICL_LANGUAGE_CODE;
                if($lanCode=='en'){
        ...
        function edit_assets(){
               ...
               $update_id = wp_update_post( $my_posts );
                ...
                if($update_id){
                echo $sucess;
                update_post_meta($update_id,'short_description',$_POST['temp_description']);
   ...
   function asset_favorite(){
    ...
    function update_reminder(){
     ...
     function accessLevel(){
       ...
       $owner_priv = $_POST['owner_priv'];
       $UserAccess = $wpdb->get_row("SELECT * FROM $table_name where user_id='$user_id' and post_id = '$post_id'");
       $u_private  = $UserAccess->user_private_note;
       ...
       if(count($UserAccess)==0){
        $insertSQL = "INSERT INTO " . $table_name . " SET post_id ='$post_id', user_id ='$user_id', user_private_note='$user_priv', user_to_owner_note='$user_owner', owner_private_note='$owner_priv'";
         $results = $wpdb->query($insertSQL);
          echo json_encode(array( 'status'=>"Success", 'class'=>"status", 'message'=>$successfully));
        }
        else if(count($UserAccess)>0){
        $updateSql = "UPDATE " . $table_name ." SET user_private_note='$user_priv', user_to_owner_note='$user_owner', owner_private_note='$owner_priv' where user_id='$user_id' and post_id='$post_id'";
        $wpdb->query($updateSql);
        echo json_encode(array( 'status'=>"Success", 'class'=>"status", 'message'=>$UpdatedMsg));
        }
        die;
       }
